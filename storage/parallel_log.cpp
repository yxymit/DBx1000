...
boost::lockfree::queue<wait_log_record> * wait_buffer(128)[NUM_LOGGER];
int * buffer_length;
...
void ParallelLogManager::checkWait(int logger_id) {
  ...
  for(int i = 0; i < buffer_length[logger_id]/(THREAD_CNT/NUM_LOGGER); i++) {
    ...
    if(my_wait_log.preds.empty()) {
      char * new_log_entry = new char[sizeof(my_wait_log.txn_id) + 4];
  ...
  int pred_size, uint64_t txn_id, int thd_id)
{
  ...
  if(my_wait_log.preds.empty()) {
    ...
    entry_size += 4;
    _logger[thd_id % THREAD_CNT].logTxn(new_log_entry, entry_size);
    // FLUSH DONE
    ...
    wait_buffer[thd_id].push(my_wait_log);
    ...
    _logger[thd_id % NUM_LOGGER].logTxn(new_log_entry, entry_size);
    buffer_length[thd_id%4]++;
    //wait_count = (wait_count + 1) % WAIT_FREQ;
    //if(wait_count == 0) {
      checkWait(thd_id%THREAD_CNT);
    //}
  }
}
